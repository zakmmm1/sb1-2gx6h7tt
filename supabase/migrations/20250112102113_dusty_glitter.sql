...
-- Step 1: Ensure owner_accounts table exists with proper constraints
CREATE TABLE IF NOT EXISTS owner_accounts (
  ...
  UNIQUE(owner_id)  -- Add unique constraint
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM owner_accounts WHERE owner_id = distinct_owners.user_id
);
